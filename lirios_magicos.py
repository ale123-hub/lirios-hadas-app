...
pygame.display.set_caption("Lirios Amarillos y Hadas de Luz")
...
# Más lirios con diferentes posiciones, alturas y escalas (x, base_y, height, scale)
lily_positions = [
    (120, 580, 160, 0.75),
    (220, 585, 210, 0.95),
    (340, 595, 240, 1.1),
...
while running:
    ...
    # Actualizar y dibujar partículas/hadas
    mouse_x, mouse_y = pygame.mouse.get_pos()
    for fairy in fairies:
        dist = math.hypot(mouse_x - fairy.x, mouse_y - fairy.y)
        if dist < 80:
            fairy.x += (mouse_x - fairy.x) * 0.02
            fairy.y += (mouse_y - fairy.y) * 0.02
