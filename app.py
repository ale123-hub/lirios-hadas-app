...
# Inicialización en el estado de la sesión para mantener las posiciones sostenidas
if "initialized" not in st.session_state:
    pygame.init()
    st.session_state.stars = [(random.randint(0, WIDTH), random.randint(0, 350), random.randint(1, 2)) for _ in range(80)]
    st.session_state.grass_blades = [
...
def draw_grass(surface):
    ...
    for x, height, bend, phase in st.session_state.grass_blades:
...
for sx, sy, ssize in st.session_state.stars:
...
# Mostrar imagen estable
st.image(img_array, width="stretch")

# Auto-refresco nativo de Streamlit para animar
st.rerun()